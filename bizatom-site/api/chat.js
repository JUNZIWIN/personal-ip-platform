...
function matchKnowledge(question, lang = 'en') {
  ...
  // Step 1: Identify which book the question is about
  let bestBook = null;
  let bestScore = 0;
  ...
  if (!bestBook || bestScore === 0) return null;

  const data = bestBook[lang] || bestBook.en;

  // Step 2: PRIORITY check — does the user ask about a SPECIFIC concept?
  // Find best-matching concept with bidirectional matching
  ...
  for (const [name, desc] of concepts) {
    ...
    const terms = nameLower.split(/[\s\-:：]+/);
    // Forward: does question contain concept name parts?
    for (const t of terms) {
      if (t.length >= 3 && q.includes(t)) cs += t.length * 3;
    }
    // Give a big bonus if the full concept name appears in the question
    if (q.includes(nameLower)) cs += nameLower.length * 5;
    // Bonus for partial concept name match (e.g. question has "MVP", concept is "最小可行产品MVP")
    const shortName = nameLower.slice(0, Math.min(8, nameLower.length));
    if (shortName.length >= 3 && q.includes(shortName)) cs += 10;
    // Reverse: does any concept name contain words from the question?
    const qWords = q.split(/\s+/);
    for (const qw of qWords) {
      if (qw.length >= 3 && nameLower.includes(qw)) cs += qw.length * 3;
    }
    // Also extract non-CJK tokens from concept name and check against question
    const asciiTokens = nameLower.match(/[a-z0-9]+/g) || [];
  ...
  // Step 3: Check QA pairs for exact match
  let bestQA = null;
  let bestQAScore = 0;
  for (const qa of data.qa) {
    const qaQ = qa.q.toLowerCase().replace(/[？?]/g, '').trim();
    let qaScore = 0;
    // Check how many significant words from QA question appear in user question
    const qaTerms = qaQ.split(/\s+/);
    for (const t of qaTerms) {
      if (t.length >= 3 && q.includes(t)) qaScore += t.length;
    }
    // Bonus for concept keywords
    for (const [cname] of concepts) {
      const cLower = cname.toLowerCase();
      if (q.includes(cLower.slice(0, 6))) qaScore += 10;
    }
    if (qaScore > bestQAScore) { bestQAScore = qaScore; bestQA = qa; }
  }

  // ─── PRIORITY OUTPUT ───
  // Threshold: if concept match score is strong, this is a CONCEPT question
  // Return JUST the concept definition with brief book attribution
  const isConceptQuestion = bestCS >= 6 || (bestQA && bestQAScore >= 6);

  if (isConceptQuestion && bestConcept) {
    let answer = `💡 **${bestConcept.name}**\n\n${bestConcept.desc}\n\n`;
    answer += `📖 *出自：${data.title}*\n`;

    // If a QA also matches strongly, append it as deeper insight
  ...
  // Step 4: If NOT a specific concept question, give the full book overview
  let answer = `📚 **${data.title}**\n\n${data.brief}\n\n`;

  // Add the top concept if one was found
  if (bestConcept && bestCS > 3) {
    answer += `💡 **${bestConcept.name}**：${bestConcept.desc}\n\n`;
  }

  // Include concepts summary (but only if not overloaded with concept-specific answer)
